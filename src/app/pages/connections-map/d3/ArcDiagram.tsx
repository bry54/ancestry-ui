...
const ArcDiagram: React.FC<ArcDiagramProps> = ({ data, width = 900, height = 600 }) => {
  ...
  useEffect(() => {
    ...
    const margin = { top: 100, right: 30, bottom: 30, left: 30 }; // Increased top margin for rotated labels

    const innerWidth = width - margin.left - margin.right;
    const innerHeight = height - margin.top - margin.bottom;

    const g = svg.append('g').attr('transform', `translate(${margin.left},${margin.top})`);
    //const g = svg.append('g').attr('transform', `translate(${100},${-margin.top})`);
    ...
    const x = d3.scalePoint()
        ...
        .range([0, innerWidth])
        .padding(0.5); // Reverted padding to a more standard value
    ...
    // Draw node labels (rotated for horizontal layout)
