...
export const ResponsiveZoomableSunburst: React.FC<ResponsiveZoomableSunburstProps> = ({ data }) => {
  // 1. Measure the container
  const { ref, width, height } = useContainerSize();
  
  // 2. Set up state to track the currently focused arc (for zooming)
  const [focusedArc, setFocusedArc] = useState<d3.HierarchyRectangularNode<HierarchyData> | null>(null);
  ...
  const { root, colorScale, arcGenerator } = useMemo(() => {
    ...
    if (!focusedArc) {
      setFocusedArc(d3.partition<HierarchyData>().size([2 * Math.PI, root.height + 1])(root));
    }

    const colorScale = d3.scaleOrdinal(d3.quantize(d3.interpolateRainbow, (root.children?.length || 0) + 1));
    const arcGenerator = d3.arc<d3.HierarchyRectangularNode<HierarchyData>>()
      .startAngle(d => d.x0)
      .endAngle(d => d.x1)
      .padAngle(d => Math.min((d.x1 - d.x0) / 2, 0.005))
      .padRadius(Math.min(width, height) / 2)
      .innerRadius(d => d.y0 * (Math.min(width, height) / 2 / (root.height + 1)))
      .outerRadius(d => Math.max(d.y0 * (Math.min(width, height) / 2 / (root.height + 1)), d.y1 * (Math.min(width, height) / 2 / (root.height + 1)) - 1));
  ...
  return (
    <div ref={ref} className="w-full h-full">
      <svg width={width} height={height} viewBox={`0 0 ${width} ${height}`} style={{ cursor: 'pointer' }}>
        <g transform={`translate(${width / 2},${height / 2})`}>
          {d3.partition<HierarchyData>().size([2 * Math.PI, root.height + 1])(root).descendants().map((p, i) => (
            <path
              key={`${p.data.name}-${i}`}
            ...
            >
              <title>{p.ancestors().map(d => d.data.name).reverse().join(" / ")}</title>
            </path>
          ))}
          <circle
            ...
            fill="none"
            pointerEvents="all"
            onClick={() => handleArcClick(d3.partition<HierarchyData>().size([2 * Math.PI, root.height + 1])(root))}
          />
        </g>
      </svg>
    </div>
